...
class CreateWoreportviewsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        DB::statement("
        CREATE VIEW get_wo_report_details AS
            ...
            AS status,(select concat(users.f_name,' ',users.l_name) from users where (orders.updated_by = users.id)) 
            AS changed_by,orders.updated_at 
    ...
    {
        DB::statement('DROP VIEW IF EXISTS get_wo_report_details');
    }
}